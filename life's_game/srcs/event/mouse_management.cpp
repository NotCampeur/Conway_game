...
void	click_manager(SDL_Event e)
{
	...
	{
		if (sys->grid[e.button.x / (TILE_WIDTH + 1)][e.button.y / (TILE_HEIGHT + 1)] == 0)
			sys->grid[e.button.x / (TILE_WIDTH + 1)][e.button.y / (TILE_HEIGHT + 1)] = 1;
		else
			sys->grid[e.button.x / (TILE_WIDTH + 1)][e.button.y / (TILE_HEIGHT + 1)] = 0;
	}
}
